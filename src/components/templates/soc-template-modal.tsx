...
export function SOCTemplateModal({ isOpen, onClose }: SOCTemplateModalProps) {
  ...
  const handleSubmit = () => {
    console.log('SOC Template Data:', formData);
    onClose();
  };
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] p-0 bg-slate-200">
        ...
        <div className="p-6 space-y-2 overflow-y-auto max-h-[calc(80vh-80px)]">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div>
              <Label htmlFor="socDate" className="text-sm font-medium">SOC Date</Label>
              <Input
                id="socDate"
                value={formData.socDate}
                onChange={(e) => setFormData({ ...formData, socDate: e.target.value })}
                className="mt-1"
              />
            </div>
            <div>
